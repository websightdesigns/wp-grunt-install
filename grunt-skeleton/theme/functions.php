<?php
/**
 * SKEL_THEME_PREFIX functions and definitions
 *
...
if ( ! function_exists( 'SKEL_THEME_PREFIX_setup' ) ) :
/**
 ...
 */
function SKEL_THEME_PREFIX_setup() {

	/*
	 * Make theme available for translation.
	 * Translations can be filed in the /languages/ directory.
	 * If you're building a theme based on SKEL_THEME_PREFIX, use a find and replace
	 * to change 'SKEL_THEME_PREFIX' to the name of your theme in all the template files
	 */
	load_theme_textdomain( 'SKEL_THEME_PREFIX', get_template_directory() . '/languages' );
	...
	register_nav_menus( array(
		'primary' => __( 'Primary Menu', 'SKEL_THEME_PREFIX' ),
	) );
	...
	add_theme_support( 'custom-background', apply_filters( 'SKEL_THEME_PREFIX_custom_background_args', array(
		'default-color' => 'ffffff',
		'default-image' => '',
	) ) );
}
endif; // SKEL_THEME_PREFIX_setup
add_action( 'after_setup_theme', 'SKEL_THEME_PREFIX_setup' );
 ...
 */
add_action( 'widgets_init', 'SKEL_THEME_PREFIX_widgets_init' );
if ( ! function_exists( 'SKEL_THEME_PREFIX_widgets_init' ) ) :
	function SKEL_THEME_PREFIX_widgets_init() {
		register_sidebar( array(
			'name'          => __( 'Sidebar', 'SKEL_THEME_PREFIX' ),
			'id'            => 'sidebar-1',
			'description'   => '',
 ...
 */
add_action( 'wp_enqueue_scripts', 'SKEL_THEME_PREFIX_scripts' );
if ( ! function_exists( 'SKEL_THEME_PREFIX_scripts' ) ) :
	function SKEL_THEME_PREFIX_scripts() {
		...
		if($hostname_match) {
			wp_enqueue_style( 'SKEL_THEME_PREFIX-style', get_template_directory_uri() . '/style.css', array(), '1.0' );
		} else {
			wp_enqueue_style( 'SKEL_THEME_PREFIX-style', get_template_directory_uri() . '/style.min.css', array(), '1.0' );
		}

		// Enqueue the theme javascript
		if($hostname_match) {
			wp_enqueue_script( 'SKEL_THEME_PREFIX-script', get_template_directory_uri() . '/js/script.js', array(), '3.1.1', array( 'colorbox' ) );
		} else {
			wp_enqueue_script( 'SKEL_THEME_PREFIX-script', get_template_directory_uri() . '/js/script.min.js', array(), '3.1.1', array( 'colorbox' ) );
		}
	}
	endif;
...
if ( ! function_exists( 'special_nav_class' ) ) :
	add_action( 'template_redirect', 'SKEL_THEME_PREFIX_change_search_url_rewrite' );
	function SKEL_THEME_PREFIX_change_search_url_rewrite() {
 ...
 */
if ( ! function_exists( 'special_nav_class' ) ) :
	add_filter('pre_get_posts','SKEL_THEME_PREFIX_search_filter');
 ...
 */
// add_filter( 'manage_users_sortable_columns', 'SKEL_THEME_PREFIX_sortable_user_table_column' );
// function SKEL_THEME_PREFIX_sortable_user_table_column( $columns ) {
 ...
 */
add_filter( 'manage_users_columns', 'SKEL_THEME_PREFIX_modify_user_table' );
if ( ! function_exists( 'SKEL_THEME_PREFIX_modify_user_table' ) ) :
	function SKEL_THEME_PREFIX_modify_user_table( $column ) {
		$column['comments'] = 'Comments';
		return $column;
	}
endif;

add_filter( 'manage_users_custom_column', 'SKEL_THEME_PREFIX_modify_user_table_row', 10, 3 );
if ( ! function_exists( 'SKEL_THEME_PREFIX_modify_user_table_row' ) ) :
	function SKEL_THEME_PREFIX_modify_user_table_row( $val, $column_name, $user_id ) {
...
add_action( 'admin_head-users.php', 'SKEL_THEME_PREFIX_users_table_style' );
if ( ! function_exists( 'SKEL_THEME_PREFIX_users_table_style' ) ) :
	function SKEL_THEME_PREFIX_users_table_style() {
		?><style>
 ...
 */
add_filter( 'manage_users_sortable_columns', 'SKEL_THEME_PREFIX_sortable_user_table_column' );
if ( ! function_exists( 'SKEL_THEME_PREFIX_sortable_user_table_column' ) ) :
	function SKEL_THEME_PREFIX_sortable_user_table_column( $columns ) {
		$columns['posts'] = 'posts';
