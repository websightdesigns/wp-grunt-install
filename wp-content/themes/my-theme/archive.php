				...
				<header class="page-header">
					<h1 class="page-title">
						<?php
							...
							elseif ( is_author() ) :
								printf( __( 'Posts by %s', 'my_theme' ), '<span class="vcard">' . get_the_author() . '</span>' );

							elseif ( is_day() ) :
								printf( __( 'Posts from %s', 'my_theme' ), '<span>' . get_the_date() . '</span>' );

							elseif ( is_month() ) :
								printf( __( 'Posts from %s', 'my_theme' ), '<span>' . get_the_date( _x( 'F Y', 'monthly archives date format', 'SKEL_THEME_PREFIX' ) ) . '</span>' );

							elseif ( is_year() ) :
								printf( __( 'Posts from %s', 'my_theme' ), '<span>' . get_the_date( _x( 'Y', 'yearly archives date format', 'SKEL_THEME_PREFIX' ) ) . '</span>' );

							elseif ( is_tax( 'post_format', 'post-format-aside' ) ) :
								_e( 'Asides', 'my_theme' );

							elseif ( is_tax( 'post_format', 'post-format-gallery' ) ) :
								_e( 'Galleries', 'my_theme' );
					...
					<ol class="breadcrumb">
						...
						<li class="active"><?php
							if( is_category() ) {
								...
								printf( __( 'Posts from %s', 'my_theme' ), get_the_date( _x( 'F Y', 'monthly archives date format', 'SKEL_THEME_PREFIX' ) ) );
							} elseif( is_day() ) {
								printf( __( 'Posts from %s', 'my_theme' ), get_the_date() );
							} elseif( is_year() ) {
								printf( __( 'Posts from %s', 'my_theme' ), get_the_date( _x( 'Y', 'yearly archives date format', 'SKEL_THEME_PREFIX' ) ) );
							} elseif( is_author() ) {
								printf( __( 'Posts by %s', 'my_theme' ), get_the_author() );
							}
						?></li>
					</ol>
				<?php } ?>
