...
function wp_customize_install() {
    ...
    // set up variable for the login page title
    $the_page_title = 'Log In';

    // set up variable for the login page slug
    $the_page_name = ( get_option('wpcustomize_admin_page_url') ? str_replace('/', '', get_option('wpcustomize_admin_page_url') ) : 'login' );
     ...
     */
    if( isset($_SERVER['SERVER_NAME']) && $_SERVER['SERVER_NAME'] ) $servername = str_replace(".", "\.", $_SERVER['SERVER_NAME']);
    ...
    $site_url_parts = explode($admin_path, $_SERVER['REQUEST_URI']);
    if( isset( $_POST['wpcustomize_admin_page_url'] ) && $_POST['wpcustomize_admin_page_url'] ) {
        $site_path = $site_url_parts[0] . $_POST['wpcustomize_admin_page_url'];
    } else {
        $site_path = $site_url_parts[0] . ( get_option('wpcustomize_admin_page_url') ? get_option('wpcustomize_admin_page_url') : '/login/' );
    }
    // $site_path_noslash = str_replace('/', '', $site_path);

    $rel_site_path = substr($site_url_parts[0], 1);

// ob_start(); echo "<pre>"; var_dump($admin_path); echo "</pre>"; $dump = ob_get_clean(); echo $dump;
// ob_start(); echo "<pre>"; var_dump($site_path); echo "</pre>"; $dump = ob_get_clean(); echo $dump;
// ob_start(); echo "<pre>"; var_dump($rel_site_path); echo "</pre>"; $dump = ob_get_clean(); echo $dump;
// die;
...
RewriteCond %{QUERY_STRING} !(?:^)reauth=1
