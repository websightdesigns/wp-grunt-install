...
function wpcustomize_login(){
	...
	if (
		...
		$redirect_url = home_url( ( get_option('wpcustomize_admin_page_url') ? get_option('wpcustomize_admin_page_url') : '/login/' ) );
...
function wpcustomize_verify_username_password( $user, $username, $password ) {
	$login_page  = home_url( ( get_option('wpcustomize_admin_page_url') ? get_option('wpcustomize_admin_page_url') : '/login/' ) );
 ...
 * ************************************************************
 */
function wpcustomize_flush_rewrites() {
	if(
		(
			isset( $_GET['page'] )
			&& 'settings' == $_GET['page']
		...
		if( isset( $_GET['action'] ) && 'deactivate' != $_GET['action'] ) wp_customize_install();
	}
}
add_filter('admin_head', 'wpcustomize_flush_rewrites');
